...
- 打印能力的固定目标是平台总开关 `Merchant.printingEnabled` + 商家打印中心配置；自动打印和执行端上线前仍默认关闭。当前平台能力字段与商家可修改开关之间的偏差见“当前实现与硬约束的差异”。
...
以下是本轮只读事实核对发现的现状偏差。本轮按范围只记录、不修改业务代码；这些项目在生产部署 Gate 前必须另行校正和回归，未校正前不得宣称打印 Gate 已满足。

1. 平台后台当前使用 capability `printerEnabled` 表达“允许打印机管理”，而打印任务中心使用 `Merchant.printingEnabled`；同时 `OWNER/MANAGER` 仍可通过 `PATCH /merchant/printing/settings` 修改后者。因而“平台开通/关闭，商家只做配置”的硬约束尚未完全落地。证据：`apps/merchant-admin/src/pages/PlatformMerchantDetailPage.vue`、`apps/api/src/modules/platform/platform-dictionary-seed.ts`、`apps/api/src/modules/printing/controllers/merchant-printing.controller.ts`、`apps/api/src/modules/printing/services/printing-settings.service.ts`。
2. Web 收银台当前 `readyUsbPrinters` 只排除 `OFFLINE`、`ERROR` 和 `DISABLED`，会让 `UNVERIFIED` 或 `UNKNOWN` 进入 READY 候选。基准要求“可以打印”必须有明确在线和可执行证据；未知、未验证状态必须按“打印设备离线/不可执行”处理。证据：`apps/merchant-cashier/src/stores/printing.ts`、`apps/api/prisma/schema.prisma`。

上述差异不需要新增订单、桌台、TableSession、员工或权限接口；后续若校正，只能收敛现有打印开关和状态判断。
...
打印能力的固定目标是“平台开通/关闭、商家配置”。当前实现状态如下，已知偏差以“部分实现”标记：

| 层级 | 责任 | 证据 |
|---|---|---|
| 平台 | `Merchant.printingEnabled` 是任务中心总 Gate，平台禁用商家时会关闭它；但平台 capability `printerEnabled` 尚未与该 Gate 完全统一，且商家端仍可修改后者，当前为部分实现 | `apps/api/prisma/schema.prisma`, `apps/api/src/modules/platform/platform-merchants.service.ts`, `apps/api/src/modules/platform/platform-dictionary-seed.ts`, `apps/api/src/modules/printing/services/printing-settings.service.ts` |
...
| Web 收银台 | 已有“未开通/未配置/设备离线/可打印”四态，但 READY 对 UNKNOWN/UNVERIFIED 的排除尚不完整，当前为部分实现 | `apps/merchant-cashier/src/stores/printing.ts`, `apps/merchant-cashier/src/i18n/messages.ts`, `apps/api/prisma/schema.prisma` |
...
| 订单与桌台功能复用现有接口 | PASS |
| 打印遵循平台开关 + 商家配置 | PARTIAL：架构约束已固定；平台/商家开关边界与 READY 判定仍需在生产部署 Gate 前校正 |

...
> 文档性质：建议方案，不代表功能已经实现。
> 当前事实基线见 `docs/printing-v1/00_CURRENT_STATE_AUDIT.md`。D10 Pro、ZY305UL、LAN、ESC/POS 和任何具体端口均未视为验证成功。
...
3. 测试打印、首次人工打印、人工补打和自动打印都先创建持久化 `PrintJob`。
...
- 当前设计阶段不修改 schema、API、Web 或 Android。
- 首个计划实现的执行通道仅为 `LOCAL_LAN_ESCPOS`，且必须先经过阶段 D 硬件验证。
- USB、云打印和厂商内置 SDK 只预留通道概念，不在 V1 首次实现。
...
flowchart TB
    ...
    Router -->|LOCAL_LAN_ESCPOS| Android[Android 本地连接器]
    Android -->|LAN / ESC/POS| LocalPrinter[门店 LAN 打印机]
...
API 不再负责连接商家 LAN。现有 `node:net.Socket` 旧路径必须在任务中心切换时关闭：`apps/api/src/modules/printers/printers.service.ts:341-367`。
...
### 3.4 Android 本地连接器（目标职责，打印部分当前未实现）
...
当前 Android 的网络监测、DataStore、诊断、协程生命周期和安全 WebView 可复用；打印与终端认证均不存在。证据：`apps/merchant-terminal-android/app/src/main/java/com/yunqiao/life/merchantterminal/MainActivity.kt:95-107,517-533`；`apps/merchant-terminal-android/app/src/main/java/com/yunqiao/life/merchantterminal/data/TerminalSettings.kt:20-95`；`apps/merchant-terminal-android/app/src/main/java/com/yunqiao/life/merchantterminal/diagnostics/DeviceDiagnostics.kt:36-140`；`apps/merchant-terminal-android/app/src/main/java/com/yunqiao/life/merchantterminal/web/TerminalWebView.kt:24-65`；`apps/merchant-terminal-android/README.md:363-388`。
...
|---|---|---|---|---|
| `LOCAL_LAN_ESCPOS` | 门店 LAN ESC/POS | 只建立枚举、配置和 Job 路由；无 executor 时拒绝启用/创建生产任务 | 阶段 D 验证后，由阶段 E Android 连接器实现 | V1 唯一计划本地通道；IP/端口和设备能力不得预先写死 |
| `LOCAL_USB_ESCPOS` | Android USB Host | 仅预留 enum/concept | 阶段 J | 后续需 USB permission、设备白名单和 detach 恢复 |
...
8. 本地 LAN config 可返回给绑定终端，但不返回给无设备管理权限的普通页面；服务器不主动连接该地址。V1 默认只允许绑定 Printer 配置中的 RFC1918 literal IPv4 和受控端口，并在服务端/Android 双重阻止 loopback、link-local、multicast、unspecified、broadcast 和公网地址；ReceiptDocument 不能携带 endpoint。若真实现场使用其他范围，必须做显式审计例外。
9. Receipt 可能含姓名、电话、地址。列表接口默认脱敏，详情按角色授权，日志不得打印完整 receipt、Token 或 credential。
10. 人工补打、任务取消、规则修改、终端撤销和 credential 轮换全部写审计日志。
11. 平台后台可看 requestId、错误码和适配器诊断；商家只看可行动的脱敏错误，不展示内部堆栈或厂商密钥。

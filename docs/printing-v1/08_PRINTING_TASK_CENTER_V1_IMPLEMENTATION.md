# 统一打印任务中心 V1 与旧链路切换实施记录

> 文档性质：阶段 C 任务中心及阶段 C.1 旧链路切换的实施范围、契约与验证记录。
> 安全基线：新自动创建、新任务执行和旧服务器 Socket/TCP 直打均默认关闭；当前没有真实打印能力。
> 验证状态：阶段 C 的既有验证结果保留在本文末尾；阶段 C.1 结果已由主执行线程按实际命令回填。migration 仅完成静态审计，未连接或变更数据库。
...
- 保留旧 `PrinterSetting`、`PrintLog`、旧 API、旧页面和 Socket 代码，但默认停止其入口与执行。
- 以“打印中心 Beta”作为 merchant-admin 唯一可见打印入口，并把旧页面 URL 安全重定向到新中心。
- 在配置层阻止旧自动直打与新自动 PrintJob 同时开启。

本轮明确不做：真实订单自动建任务、worker、公开 connector 领取接口、Android 终端配对、LAN/USB/内置打印执行、云打印适配、旧数据迁移或物理删除、生产 migration、部署或 push。
...
6. 阶段 C.1 不删除阶段 C 前已存在的旧实现，而是以 `LEGACY_PRINTING_ENABLED=false` 阻断自动、测试和手动执行；详细切换、回滚和物理删除准入见 `docs/printing-v1/09_LEGACY_PRINTING_CUTOVER_V1.md`。
...
| 环境变量 | 默认/本轮状态 | 作用 |
|---|---|---|
| `PRINTING_TASK_CENTER_ENABLED` | `true` | 控制 Beta 任务中心管理能力；开启不代表可以真实打印 |
...
| `LEGACY_PRINTING_ENABLED` | `false` | 是否允许旧订单自动直打、旧测试打印及旧订单手动打印；默认关闭 |

四个开关由打印配置服务集中、严格解析。即使管理能力被打开，新自动创建和执行仍必须独立保持 `false`。当 `LEGACY_PRINTING_ENABLED=true` 与 `PRINTING_AUTO_CREATE_ENABLED=true` 同时出现时，配置校验必须抛出 `PRINTING_DUAL_PATH_NOT_ALLOWED` 并阻止启动；不得让同一订单事件同时走旧 Socket 和新 PrintJob。merchant-admin 必须显示“执行端待接入”，不能显示在线、正常或已连接。

merchant-admin 不维护第二个可能与服务端漂移的环境开关。它从已认证的 `GET /merchant/printing/feature-state` 获取同一状态，只使用返回的安全布尔值控制旧 UI；请求失败时 fail-closed 为 `legacy=false`。接口不会返回环境变量原文、打印机连接配置或凭据。

## 8. Merchant API 契约

所有路由复用现有 merchant JWT 和角色 Guard；所有 ID 查询、关联写入和状态操作必须再按 `merchantId` 过滤。跨商家资源不得因知道 ID 而被读取或修改。

阶段 C.1 新增已认证只读接口 `GET /merchant/printing/feature-state`，返回四个打印 Feature Flag 布尔状态及安全的 `executionState`。这是 merchant-admin 控制旧 UI 的唯一状态来源；请求失败时前端默认旧入口关闭。接口不返回打印机 IP、连接配置、凭据或环境变量原文。
...
新入口是默认唯一可见的打印入口。旧客户端、组件和历史页面文件仍保留，但旧菜单默认隐藏；旧打印 URL 重定向到 `/printing-center/printers?from=legacy`。新中心路由为：
...
从旧 URL 进入时还需显示一次“旧打印配置入口已停用，当前已切换到打印中心 Beta”的说明。merchant-admin 订单详情不再提供可执行的旧打印按钮。UI 需要覆盖中文、越南语、英文、空状态与 1280×800；实现证据位于 `apps/merchant-admin/src/` 的 API、路由、页面、布局和 i18n 文件。
...
- `LEGACY_PRINTING_ENABLED=false` 时订单正常创建但旧 Service 不调用、不解析 IP、不创建 Socket 或旧 `PrintLog`；`true` 时保留原调用。
- `/merchant/printers` 的 GET/POST/PATCH/DELETE/test 全组接口与旧订单打印关闭时返回 `410 Gone / LEGACY_PRINTING_DISABLED`，不读取旧配置且没有发送副作用。
- 旧自动与新自动配置同时开启时以 `PRINTING_DUAL_PATH_NOT_ALLOWED` 阻止启动；新 printing API 不受旧开关影响。

### 11.3 merchant-admin 与回归

- merchant-admin typecheck/build、五个 Beta 路由、唯一打印菜单、旧 URL 重定向、切换提示、订单打印禁用、三语言和 1280×800 截图。
- 旧打印页面文件保留，但默认不可从菜单进入；Feature Flag 回滚路径需单独验证。
- API typecheck/test/build。
- merchant-cashier typecheck/lint/test/test:ui/build，确认仍显示“打印待接入”、桌账打印仍禁用且订单/桌台流程未变化。
...
| 旧打印页面回归 | PASS | 阶段 C 基线截图完成且当时旧路由/API 未变；阶段 C.1 有意切换其入口与执行行为，以后续表格为准 |

### 12.1 阶段 C.1 验证结果

以下项目已由阶段 C.1 主执行线程按最终代码实际验证；命令没有连接或写入数据库，也没有连接打印设备：

| 验证项 | 结果 | 实际记录 |
|---|---|---|
| API typecheck/test/build | PASS | 19 个测试套件、222 项测试全部通过；覆盖旧自动开/关、410、无 Socket/PrintLog 副作用、双路径保护、新 printing API 与订单创建回归 |
| merchant-admin typecheck/build | PASS | 唯一新菜单、旧 URL 重定向、切换提示、旧打印状态/按钮不可执行、三语言通过；构建仅保留既有单 chunk 超过 500 kB warning |
| merchant-cashier typecheck/lint/test/test:ui/build | PASS | 14 个测试文件、74 项测试通过；UI 脚本覆盖 7 种视口、三语言、禁用打印、订单流程和网络恢复；业务代码无本轮 diff |
| 1280×800 切换截图 | PASS | 8 张规定截图均为 1280×800 脱敏测试/空状态，不含凭据或顾客隐私，未见布局重叠 |
| Git/范围审计 | PASS | `git diff --check` 通过；无 Prisma/migration、Android、真实打印调用、密钥、环境文件或构建产物进入变更 |

`merchant-cashier test:ui` 第一次直接执行时因 Fixture Vite 前置服务尚未启动而得到 `ERR_CONNECTION_REFUSED`；按项目 README 启动 `VITE_CASHIER_USE_FIXTURES=true` 的本地 5176 服务后重新执行通过，随后已停止该本地服务。Fixture 不访问真实 API 或数据库。
...
- 用户已确认当前没有真实商家使用旧本地局域网打印；此确认不等同于生产数据库统计。
- 旧 Controller、Service、DTO、Socket 代码、`PrinterSetting` 与 `PrintLog` 仍在，只是默认停止入口与执行。
- 当前没有 Android、LAN、USB、内置或云打印执行端，新自动创建与执行保持关闭。
- 物理删除前必须完成生产旧配置数量审计及 `docs/printing-v1/09_LEGACY_PRINTING_CUTOVER_V1.md` 中的全部准入条件。

阶段 C 只创建 migration 文件；截至阶段 C.1 收口前未执行任何本地、测试或生产数据库 migration。最终验证表已由主线程根据实际命令结果回填；“文件已创建”不代表“数据库已迁移”。

下一步只能是：在隔离数据库验证新 migration，并到店完成 LAN 硬件验证；随后开发 Android LAN ESC/POS 连接器。本文不授权自动进入该阶段。

## 14. 后续方向变更附记（2026-07-15）

上一段是阶段 C/C.1 收口时的原始下一步记录，保留用于追溯；其 LAN-first 顺序已被后续确认的 USB-first 决策取代，不再是当前实施指令。
...
只有上述实机验证通过后，才在单独阶段把已验证 adapter 接入 Terminal 注册/认证、服务端配置、`PrintJob` 领取/租约/结果回报，形成正式 Android USB ESC/POS connector/executor。当前后端对 USB 仍只有枚举/概念预留，通道配置与执行尚未实现；Android 已有诊断/合成 smoke 代码，但尚无正式 PrintJob USB connector。本文不预设设备型号、设备标识、接口、端点或打印能力，也不授权自动开始该正式连接器阶段。

LAN ESC/POS 没有被删除，只降为后续通道；未来启用前仍须单独完成现场网络、地址/端口、安全边界和真实出纸验证。

...
> 决策更新（2026-07-15）：首个新本地执行通道已由 LAN 改为 USB ESC/POS。以下准入条件按当前 USB-first 方向解释；旧 LAN 配置与迁移事实仍保留，未来启用 LAN 时必须另做现场网络和硬件验证。

现有 Android 终端应用已经加入隔离的 USB 诊断与合成 smoke adapter 代码，但尚未安装到目标终端完成真实打印确认，也没有接入 `PrintJob`；因此当前仍不满足以下迁移准入条件。

只有同时满足以下条件，才允许另开阶段和分支迁移真实商家：

1. 目标 Android 终端与目标 USB ESC/POS 打印机已完成独立实机验证，包括 USB Host、系统授权、接口/端点、插拔恢复和真实出纸；不得把枚举、概念预留或未经验证的设备参数当作可用能力。

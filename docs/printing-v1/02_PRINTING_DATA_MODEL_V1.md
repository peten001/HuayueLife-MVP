...
> 历史基线说明：第 1 节记录的是本设计文档编写时的仓库快照；后续打印任务中心实现与隔离 migration 验证见 `08_PRINTING_TASK_CENTER_V1_IMPLEMENTATION.md` 和 `10_ISOLATED_MIGRATION_VALIDATION.md`，不得把第 1 节误读为当前代码仍无新模型。
> 决策更新（2026-07-15）：首个本地通道改为 USB ESC/POS。以下 LAN 配置仍作为后续通道和旧数据迁移示例保留，不代表 LAN 继续优先，也不代表 USB 后端已经实现。
...
| `name` | 必填 | 商家可识别名称 |
| `channelType` | 必填 enum | `LOCAL_USB_ESCPOS`、`LOCAL_LAN_ESCPOS` 等；首个计划本地通道为 USB |
...
以下本地 LAN `connectionConfig` 是后续通道示例，仅表示结构，不表示硬件已确认或当前优先实现：
...
| `name` | 如“前台终端-01”；不把收银机型号写入默认命名规则 |
...
| `revokedAt` | 撤销时间 |
| `capabilities` | USB Host/权限、raster、纸宽等经实机验证的终端能力 JSON；后续可扩展 LAN/厂商 SDK，USB 设备标识不得预设 |
...
| `adapter` | 首通道如 `ANDROID_USB_ESCPOS_RASTER_V1`；后续 adapter 使用独立标识 |
...
| `networkInfo` | 沿用模型字段名；本地 USB 可记录 USB Host/接口/传输的非敏感摘要，后续 LAN/云可记录网络类型与连接时间 |

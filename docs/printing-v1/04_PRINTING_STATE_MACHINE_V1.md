...
> 历史基线与决策更新：上句记录本文最初编写时的状态；打印任务中心后续实现见 `08_PRINTING_TASK_CENTER_V1_IMPLEMENTATION.md`，生产 migration 仍未执行。自 2026-07-15 起首个本地通道改为 USB ESC/POS；状态机保持通道无关，以下 LAN/TCP 场景作为后续通道保留，当前 USB smoke 不接 PrintJob。
...
| Android 目标终端重启 | 保留 | 启动后认证、心跳、读取本地台账并对账 | 开机恢复在阶段 G，阶段 E 不假设已实现 |
...
| USB 权限缺失或设备拔出 | 保留 | 建连前停止并提示授权/重连；写入中拔出按已写字节和阶段标记失败或结果不确定 | 不自动授权、不自动重打；重新绑定必须人工确认真实设备 |
| 打印机 IP 改变（后续 LAN） | 保留 | 旧地址失败；状态页提示重新发现/修改配置；待处理任务执行时可读取经授权的新 endpoint | 不做全网段侵入扫描；Attempt记录实际configVersion；render profile不兼容时不静默执行 |
...
普通 ESC/POS 的 USB bulkTransfer 或后续 LAN RAW Socket 通常都没有可依赖的、与每张小票绑定的物理完成回执。即使字节写入完成，也可能未出纸；即使连接随后断开，纸张也可能已经输出。因此软件无法同时保证“绝不漏打”和“绝不重复”这两个绝对目标。
...
6. 后续若硬件提供可靠状态查询，只能把它作为更强证据，仍需按实际目标打印机验证，不能预设任何型号、USB 接口或状态能力已支持。

...
<template>
  <div class="menu-page mx-catalog-page">
    ...
    <MerchantDialog :open="showProductModal" :title="productModalTitle" variant="drawer" @close="closeProductModal">
      <form id="merchant-product-editor" class="mx-product-editor" @submit.prevent="saveProduct">
      ...
      <template #footer><div class="mx-editor-actions"><button type="button" class="secondary" @click="closeProductModal">{{ t('cancel') }}</button><button type="submit" form="merchant-product-editor" class="primary-action">{{ productForm.id ? t('saveChanges') : t('addProduct') }}</button></div></template>
    </MerchantDialog>
    <div

...
function receiptTypeLabel(type: PrintingReceiptType) { return type === 'TABLE_BILL' ? p('checkoutReceipt') : p('customerReceipt'); }
function settingSnapshot() { return JSON.stringify({ settings: { ...receiptSettings }, paperWidth: paperWidth.value }); }
...
function resetForm() {
  Object.assign(form, { id: '', name: '', receiptType: 'ORDER_CUSTOMER', paperWidth: 'MM80', enabled: false });
  definitionText.value = JSON.stringify({ schemaVersion: 1, sections: [{ type: 'MERCHANT_HEADER' }, { type: 'ORDER_INFO' }, { type: 'ITEMS' }, { type: 'TOTALS' }, { type: 'FOOTER' }] }, null, 2);
}
function openCreate() { resetForm(); modalOpen.value = true; }
function openEdit(row: PrintingReceiptTemplate) { Object.assign(form, { id: row.id, name: row.name, receiptType: row.receiptType, paperWidth: row.paperWidth, enabled: row.enabled }); definitionText.value = JSON.stringify(row.definition, null, 2); modalOpen.value = true; }
...
const settingGroups = [
  { title: 'merchantInfoGroup', items: [{ key: 'merchantName', label: 'merchantNameLabel', hint: 'merchantNameHint', disabled: false }, { key: 'phone', label: 'merchantPhoneLabel', hint: 'phoneMissingHint', disabled: true }, { key: 'qrCode', label: 'merchantQrLabel', hint: 'qrMissingHint', disabled: true }] },
  { title: 'orderInfoGroup', items: [{ key: 'orderNumber', label: 'orderNumberLabel', hint: 'orderNumberHint', disabled: false }, { key: 'tableNumber', label: 'tableNumberLabel', hint: 'tableNumberHint', disabled: false }, { key: 'orderTime', label: 'orderTimeLabel', hint: 'orderTimeHint', disabled: false }, { key: 'note', label: 'orderNoteLabel', hint: 'orderNoteHint', disabled: false }] },
  { title: 'productsAmountsGroup', items: [{ key: 'itemPrice', label: 'itemPriceLabel', hint: 'itemPriceHint', disabled: false }, { key: 'total', label: 'orderTotalLabel', hint: 'orderTotalHint', disabled: false }] },
] as const;
</script>

<template>
  <section class="printing-panel receipt-settings-page">
    <div class="printing-toolbar receipt-settings-page__toolbar"><div class="printing-toolbar__copy"><h2>{{ p('receiptSettingsTab') }}</h2><p>{{ p('receiptSettingsSubtitle') }}</p></div><button class="printing-button printing-button--secondary" type="button" @click="load">{{ p('refresh') }}</button></div>
    ...
    <div class="receipt-settings-layout">
      <section class="receipt-settings-card">
        <div class="receipt-settings-card__intro"><div><span class="receipt-settings-eyebrow">{{ p('customerReceipt') }}</span><h3>{{ p('displayContent') }}</h3><p>{{ p('displayContentHint') }}</p></div><span v-if="supportedReceiptTypes.length > 1" class="receipt-type-list"><span v-for="type in supportedReceiptTypes" :key="type">{{ receiptTypeLabel(type) }}</span></span></div>
        <div v-for="group in settingGroups" :key="group.title" class="receipt-settings-group"><h4>{{ p(group.title as never) }}</h4><div class="receipt-settings-group__rows"><button v-for="item in group.items" :key="item.key" class="receipt-setting-row" :class="{ 'is-disabled': item.disabled }" type="button" :disabled="item.disabled" @click="receiptSettings[item.key] = !receiptSettings[item.key]"><span class="receipt-setting-row__copy"><strong>{{ p(item.label as never) }}</strong><small>{{ p(item.disabled ? item.hint as never : item.hint as never) }}</small></span><span class="receipt-switch" :class="{ 'is-on': receiptSettings[item.key] }" aria-hidden="true"><span /></span></button></div></div>
    ...
    <div class="printing-table-wrap"><table class="printing-table"><thead><tr><th>{{ p('name') }}</th><th>{{ p('receiptType') }}</th><th>{{ p('paperWidth') }}</th><th>{{ p('bilingualReceipt') }}</th><th>{{ p('version') }}</th><th>{{ p('status') }}</th><th>{{ p('updatedAt') }}</th><th>{{ p('actions') }}</th></tr></thead><tbody><tr v-for="row in rows" :key="row.id"><td><strong>{{ row.name }}</strong><small v-if="!row.merchantId">{{ p('systemDefault') }}</small></td><td>{{ receiptTypeLabel(row.receiptType) }}</td><td>{{ row.paperWidth === 'MM58' ? '58 mm' : '80 mm' }}</td><td>{{ p('bilingualReceipt') }}</td><td>v{{ row.version }}</td><td><span :class="['printing-badge', row.enabled ? 'printing-badge--success' : '']">{{ row.enabled ? p('enabled') : p('disabled') }}</span></td><td>{{ new Date(row.updatedAt).toLocaleString() }}</td><td><div class="printing-actions"><button class="printing-button printing-button--secondary printing-button--small" type="button" :disabled="!row.merchantId" @click="openEdit(row)">{{ p('edit') }}</button><button class="printing-button printing-button--secondary printing-button--small" type="button" @click="duplicate(row)">{{ p('duplicate') }}</button></div></td></tr><tr v-if="!loading && !rows.length"><td class="printing-empty" colspan="8">{{ p('noData') }}</td></tr><tr v-if="loading"><td class="printing-empty" colspan="8">{{ p('loading') }}</td></tr></tbody></table></div>
  </section>
  <div v-if="restoreConfirmOpen" class="printing-modal-backdrop" @click.self="restoreConfirmOpen = false"><div class="printing-modal receipt-confirm-modal"><header class="printing-modal__header"><h2>{{ p('restoreDefaultsTitle') }}</h2></header><div class="printing-modal__body"><p class="printing-hint">{{ p('restoreDefaultsDescription') }}</p></div><footer class="printing-modal__footer"><button class="printing-button printing-button--secondary" type="button" @click="restoreConfirmOpen = false">{{ p('cancel') }}</button><button class="printing-button" type="button" @click="restoreDefaults">{{ p('restoreDefaultsConfirm') }}</button></footer></div></div>
  <div v-if="modalOpen" class="printing-modal-backdrop" @click.self="closeModal"><form class="printing-modal" @submit.prevent="save"><header class="printing-modal__header"><h2>{{ form.id ? p('editTemplate') : p('addTemplate') }}</h2></header><div class="printing-modal__body"><label class="printing-field printing-field--full">{{ p('name') }}<input v-model="form.name" required maxlength="80" /></label><label class="printing-field">{{ p('receiptType') }}<select v-model="form.receiptType"><option value="ORDER_CUSTOMER">{{ p('customerReceipt') }}</option><option value="TABLE_BILL">{{ p('checkoutReceipt') }}</option></select></label><label class="printing-field">{{ p('paperWidth') }}<select v-model="form.paperWidth"><option value="MM58">58 mm</option><option value="MM80">80 mm</option></select></label><p class="printing-hint printing-field--full">{{ p('bilingualReceiptHint') }}</p><label class="printing-check"><input v-model="form.enabled" type="checkbox" />{{ p('enabled') }}</label><details class="printing-advanced-template printing-field--full"><summary>{{ p('advancedTemplate') }}</summary><p class="printing-hint">{{ p('advancedTemplateWarning') }}</p><label class="printing-field">{{ p('definition') }}<textarea v-model="definitionText" required spellcheck="false"></textarea></label></details><p class="printing-hint printing-field--full">{{ p('templateSnapshotHint') }}</p></div><footer class="printing-modal__footer"><button class="printing-button printing-button--secondary" type="button" @click="closeModal">{{ p('cancel') }}</button><button class="printing-button" type="submit" :disabled="saving">{{ saving ? p('saving') : p('save') }}</button></footer></form></div>
</template>

<style scoped>
.receipt-settings-page { display: grid; gap: 16px; }
.receipt-settings-page__toolbar { margin-bottom: 0; }
.receipt-settings-layout { display: grid; grid-template-columns: minmax(0, 1.35fr) minmax(300px, .85fr); gap: 16px; align-items: start; }
...
.receipt-settings-card { padding: 20px; }
.receipt-settings-card__intro, .receipt-preview-card__heading { display: flex; justify-content: space-between; gap: 16px; align-items: flex-start; }
.receipt-settings-card__intro h3, .receipt-preview-card h3 { margin: 2px 0 5px; font-size: 18px; }
.receipt-settings-card__intro p, .receipt-preview-card p { margin: 0; color: var(--printing-muted); font-size: 13px; line-height: 1.5; }
.receipt-settings-eyebrow { color: var(--printing-green); font-size: 11px; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; }
.receipt-type-list { display: flex; flex-wrap: wrap; gap: 5px; color: var(--printing-green); font-size: 12px; font-weight: 700; }
.receipt-type-list span { padding: 5px 8px; border-radius: 999px; background: #eaf6ee; }
.receipt-settings-group { display: grid; gap: 8px; margin-top: 20px; }
.receipt-settings-group h4 { margin: 0; color: var(--printing-ink); font-size: 14px; }
.receipt-settings-group__rows { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; }
.receipt-setting-row { display: flex; min-width: 0; min-height: 60px; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 12px; border: 1px solid #e1e9e3; border-radius: 10px; color: var(--printing-ink); background: #fff; text-align: left; cursor: pointer; }
...
.receipt-setting-row__copy { display: grid; gap: 3px; min-width: 0; }
.receipt-setting-row__copy strong { font-size: 13px; }
.receipt-setting-row__copy small { color: var(--printing-muted); font-size: 11px; line-height: 1.35; }
...
.receipt-footer-field { display: grid; grid-template-columns: minmax(150px, .55fr) minmax(0, 1fr) auto; align-items: center; gap: 12px; padding: 12px; border: 1px solid #e1e9e3; border-radius: 10px; }
.receipt-footer-field > span { display: grid; gap: 3px; }
.receipt-footer-field strong { font-size: 13px; }
.receipt-footer-field small { color: var(--printing-muted); font-size: 11px; }
.receipt-footer-field input { min-height: 42px; min-width: 0; padding: 0 11px; border: 1px solid var(--printing-border); border-radius: 8px; color: var(--printing-ink); font: inherit; }
.receipt-footer-field em { color: var(--printing-muted); font-size: 11px; font-style: normal; white-space: nowrap; }
.receipt-preview-card { position: sticky; top: 16px; overflow: hidden; }
.receipt-preview-card__heading { padding: 18px 18px 14px; border-bottom: 1px solid #e8eeea; }
...
.receipt-preview-stage { display: grid; min-height: 420px; place-items: start center; padding: 22px 14px; overflow: auto; background: #f1f5f2; }
.receipt-paper { flex: 0 0 auto; min-height: 370px; padding: 24px 16px; color: #1b1b1b; background: #fff; box-shadow: 0 8px 20px rgba(18,45,29,.16); font: 12px/1.5 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
.receipt-paper--58 { width: min(260px, 100%); }
.receipt-paper--80 { width: min(340px, 100%); }
.receipt-paper__merchant { margin-bottom: 18px; text-align: center; font-size: 16px; font-weight: 800; }
.receipt-paper__meta { display: grid; gap: 5px; }
...
.receipt-paper__divider { height: 1px; margin: 16px 0 12px; background: repeating-linear-gradient(90deg, #444 0 4px, transparent 4px 7px); }
.receipt-paper__items { display: grid; gap: 9px; }
.receipt-paper__item strong, .receipt-paper__total strong { text-align: right; white-space: nowrap; }
.receipt-paper__note { margin-top: 14px; color: #333; }
.receipt-paper__note span { margin-right: 7px; }
.receipt-paper__total { margin-top: 18px; padding-top: 12px; border-top: 1px solid #555; font-size: 14px; font-weight: 800; }
.receipt-paper__footer { margin-top: 28px; color: #555; text-align: center; white-space: pre-wrap; overflow-wrap: anywhere; }
.receipt-settings-actionbar { display: flex; justify-content: space-between; gap: 12px; padding: 14px 0 0; border-top: 1px solid var(--printing-border); }
.receipt-settings-actionbar > div { display: flex; gap: 8px; }
.receipt-confirm-modal { width: min(440px, 100%); }
.receipt-confirm-modal .printing-modal__body { display: block; }
@media (max-width: 900px) { .receipt-settings-layout { grid-template-columns: 1fr; } .receipt-preview-card { position: static; } }
@media (max-width: 600px) { .receipt-settings-card { padding: 14px; } .receipt-settings-card__intro, .receipt-preview-card__heading { flex-direction: column; } .receipt-settings-group__rows { grid-template-columns: 1fr; } .receipt-footer-field { grid-template-columns: 1fr auto; } .receipt-footer-field input { grid-column: 1 / -1; grid-row: 2; } .receipt-settings-actionbar { align-items: stretch; flex-direction: column; } .receipt-settings-actionbar > div { display: grid; grid-template-columns: 1fr 1fr; } .receipt-settings-actionbar > div .printing-button:last-child { grid-column: 1 / -1; } .receipt-preview-stage { min-height: 360px; padding: 18px 8px; } }
</style>

...
桌面使用单行顶部品牌栏和直接导航：`首页 / 菜品 / 订单 / 结账 / 桌台 / 店铺设置 / 打印中心 / 员工管理`；右侧仅保留语言切换与退出账号。各入口仍按角色和能力筛选。取消电脑端“更多”、重复桌台入口、账号设置快捷入口与“返回更多”；旧 `/more` 地址返回首页，STAFF 返回店铺设置以避免无权限首页跳转循环。页面内部以紧凑工具栏、目录或筛选、连续内容区和逐层详情组织，不把所有业务都套成 KPI 卡片。

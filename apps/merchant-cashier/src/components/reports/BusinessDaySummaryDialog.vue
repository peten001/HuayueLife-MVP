      ...
      <template v-else-if="summary">
        ...
        <section class="summary-section">
          ...
          <div v-if="summary.itemSummary.length" class="summary-items">
            <div v-for="item in summary.itemSummary" :key="`${item.nameZh}-${item.nameVi}`"><span :title="[item.nameZh, item.nameVi, item.nameEn].filter(Boolean).join(' / ')">{{ item.nameZh }}</span><strong>× {{ item.quantity }}</strong></div>
          </div>

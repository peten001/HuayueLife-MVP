...
enum class TerminalLoadErrorType(
    ...
    CONFIGURATION(
        ...
        descriptionZh = "请在构建时配置可信的 HTTPS 收银台地址和精确 Origin。",
        descriptionVi = "Hãy cấu hình URL HTTPS và Origin tin cậy khi xây dựng.",
    ),
    PAGE_LOAD_FAILURE(
        code = "E_PAGE_LOAD_FAILURE",
        titleZh = "页面加载失败",
        titleVi = "Không thể tải trang",
        descriptionZh = "请检查网络后重新加载；如持续失败，请联系技术支持。",
        descriptionVi = "Hãy kiểm tra mạng và tải lại; nếu lỗi tiếp diễn, hãy liên hệ hỗ trợ.",
    ),
}

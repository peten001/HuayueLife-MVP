...
object MerchantWebSessionContract {
    ...
    fun isOpenPrinterDevicesMessage(value: String?): Boolean {
        if (value.isNullOrBlank() || value.length > 160) return false
        val json = runCatching { JSONObject(value) }.getOrNull() ?: return false
        if (json.length() != 2) return false
        return json.optString("type") == "OPEN_PRINTER_DEVICES" &&
            json.optInt("version", -1) == 1
    }

    fun sessionTransitionSignal(previous: Boolean, current: Boolean): String? = when {
        previous && !current -> SIGN_OUT_MESSAGE
        !previous && current -> SESSION_CHANGED_MESSAGE
        else -> null
    }

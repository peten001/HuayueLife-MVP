...
仓库当前不能安全确认正式线上地址，因此默认值是不可解析的安全占位地址：

```text
https://merchant-admin.invalid/
```

默认 APK 不会误连到猜测的生产服务。构建可通过 Gradle property 或环境变量注入配置；URL 必须是 HTTPS，可信 Host 只写主机名，不写 scheme、路径或密码。

仓库现有前端配置可确认 API Host 为 `api.huayueyouxuan.com`，因此它是 debug/release 的默认额外可信 Host。起始 merchant-admin URL 仍未猜测；如果构建参数覆盖可信 Host，必须继续包含实际使用的 API Host。
...
```text
yunqiao-merchant-terminal-v0.1.0-debug.apk
```
...
```bash
export MERCHANT_ADMIN_URL_RELEASE='https://merchant.example.invalid/'
export MERCHANT_TRUSTED_HOSTS_RELEASE='api.example.invalid,media.example.invalid'
...
```bash
adb devices -l
adb install -r app/build/outputs/apk/debug/yunqiao-merchant-terminal-v0.1.0-debug.apk
```
...
adb shell dumpsys package com.yunqiao.life.merchantterminal
adb logcat
```

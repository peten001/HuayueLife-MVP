...
到店前服务器任务执行、商家打印总开关、打印机开关、本机连接器开关和自动打印均保持
关闭。保存 USB 配置、安装 APK 或登录 Web 收银台不会自动开启打印。
...
- Foreground Service、BootReceiver、受控 USB attach 和 WorkManager 均重新检查商家会话、
  本地开关、远端开关、USB 绑定和权限，任何条件不满足都不执行。
...
本仓库 Gate 只完成源码、单元测试、lint、正式签名构建及二进制审计。本轮不安装收银机、
不请求 USB 权限、不开启商家打印、不创建打印任务、不真实出纸。

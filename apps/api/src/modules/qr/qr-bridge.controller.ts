...
export class QrBridgeController {
  ...
  async bridge(@Param('token') token: string, @Res() response: Response) {
    ...
    const fallbackUrl = `https://api.huayueyouxuan.com/api/v1/qr/resolve?token=${encodeURIComponent(token)}`;
  ...
  <body>
    <div class="wrap">
      <div class="card">
        ...
        <div><a class="secondary" href="${this.escapeAttr(fallbackUrl)}" rel="noreferrer">若未自动打开，请点击此处继续</a></div>
